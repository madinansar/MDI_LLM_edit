...
class Config:
    ...
    def from_name(cls, name: str, **kwargs: Any) -> Self:
        ...
        if name not in name_to_config:
            # search through all `config['hf_config']['name']` (including formatted versions)
            conf_dict = None
            for config in configs:
                hf_name = config["hf_config"]["name"]
                # Try exact match first
                if name == hf_name:
                    conf_dict = config
                    break
                # Try formatted versions if hf_name contains {}
                if "{}" in hf_name:
                    if name == hf_name.format("") or name == hf_name.format("-Instruct"):
                        conf_dict = config
                        break
                # Also check if name matches the config name field
                if name == config["name"]:
                    conf_dict = config
                    break
