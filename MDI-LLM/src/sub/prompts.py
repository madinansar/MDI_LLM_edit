...
def get_user_prompt(prompt: str, n_samples: int = 1, prompt_style: PromptStyle = NoPrompt, **kwargs) -> List[str]:
    """
    Extract the user prompt and apply the prompt style.
    ...
    if prompt.startswith("FILE:"):
        ...
        # Apply prompt style
        out = [prompt_style.apply(p) for p in out]

        if verb:
            print(out)
        return out
    else:
        return [prompt_style.apply(prompt)] * n_samples
